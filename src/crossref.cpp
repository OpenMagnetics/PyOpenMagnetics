...
// mirror that surface, following the same {"data": ...} success envelope as the
// advisers; failures raise PyOpenMagnetics.EngineError (ABT #595).
...
void register_crossref_bindings(py::module& m) {
    m.def("calculate_cross_referenced_core", &calculate_cross_referenced_core,
        ...
        Returns:
            {"data": [{"core", "scoring", "scoringPerFilter"}]} best first.
    ...
    m.def("calculate_cross_referenced_core_material", &calculate_cross_referenced_core_material,
        ...
        Returns:
            {"data": [{"material", "scoring", "scoringPerFilter"}]} best first.
            Raises PyOpenMagnetics.EngineError on failure.
        )pbdoc",
